...
**Code Cloner** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Code-Cloner --path ./target

# With options
./Code-Cloner --path ./target --format json --output report.json

# Verbose mode
./Code-Cloner --path ./target --verbose
```
...
### Basic scan
```bash
./Code-Cloner --path ./my-project
```

### JSON report
```bash
./Code-Cloner --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Code Cloner
  run: ./Code-Cloner --path . --format json --output report.json
```
